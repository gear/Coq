...
Check (beq_nat (plus 2 3) (1 + 4)).
Eval simpl in (beq_nat (plus 2 3) (1 + 4)).
(* Variable x : nat. *)

(* Eval simpl in beq_nat (plus 1 x) (plus x 1). *)
(* Goal (beq_nat (x + 1) (1 + x) = true). *)
(* Proof. *)
(*   induction x. reflexivity. *)
(*   simpl. simpl in IHn. exact IHn. *)
(* Qed. *)

Lemma plus_comm' : forall (x y : nat), beq_nat (x + y) (y + x) = true.
Proof.
  intros m n.
  induction m. simpl.
